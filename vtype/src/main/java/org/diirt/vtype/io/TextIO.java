...
public class TextIO {
    ...
    public static VType readList(Reader reader) throws IOException {
        ...
        if (numbers) {
            ...
            return VType.toVType(numberList);
        } else {
            return VType.toVType(lines);
        }
    }
    ...
    public static void writeList(VType vType, Writer writer) throws IOException {
        ...
        if (vType instanceof VNumberArray) {
            ...
            throw new UnsupportedOperationException("Can't serialize " + VType.typeOf(vType).getSimpleName() + " to a list");
        }
        out.flush();
    }
