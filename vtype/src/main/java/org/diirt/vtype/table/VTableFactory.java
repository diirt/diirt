...
import org.epics.util.array.BufferInteger;
...
public class VTableFactory {
    ...
    public static VTable join(VTable... tables) {
        ...
        // Find rows
        boolean done = false;
        List<BufferInteger> rowIndexes = new ArrayList<>();
        for (int i = 0; i < tables.length; i++) {
            rowIndexes.add(new BufferInteger());
        ...
        return VTable.of(columnTypes, columnNames, columnData);
    }
    ...
    public static VTable union(VString extraColumnName, final VStringArray extraColumnData, final VTable... tables) {
        ...
        final ListInteger offsets = ArrayInteger.of(tableOffsets);
    ...
    private static Object selectColumnData(VTable table, int column, ListInteger indexes) {
    ...
    private static <T> List<T> createView(final List<T> list, final ListInteger indexes) {
    ...
    private static Object createView(final Object columnData, final ListInteger indexes) {
    ...
    public static VTable select(final VTable table, final ListInteger indexes) {
    ...
    private static VTable extractRows(VTable vTable, ListInteger indexes) {
        ...
        if (values.get(0) instanceof VNumber) {
            for (VType vType : values) {
                if (!(vType instanceof VNumber)) {
                    throw new IllegalArgumentException("Values do not match (VNumber and " + VType.typeOf(vType).getSimpleName());
                }
            }
            return valueNumberTable(names, values);
        }

        throw new IllegalArgumentException("Type " + VType.typeOf(values.get(0)).getSimpleName() + " not supported for value table");
    }
    ...
    private static VTable valueNumberTable(List<String> names, List<? extends VType> values) {
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            severity.add(vNumber.getAlarm().getSeverity().toString());
            status.add(vNumber.getAlarm().getName());
        }

        if (names == null) {
            return newVTable(column("Value", VDoubleArray.of(ArrayDouble.of(data), Alarm.none(), Time.now(), Display.none())),
                    column("Severity", VStringArray.of(severity, Alarm.none(), Time.now())),
                    column("Status", VStringArray.of(status, Alarm.none(), Time.now())));
        } else {
            return newVTable(column("Name", VStringArray.of(names, Alarm.none(), Time.now())),
                    column("Value", VDoubleArray.of(ArrayDouble.of(data), Alarm.none(), Time.now(), Display.none())),
                    column("Severity", VStringArray.of(severity, Alarm.none(), Time.now())),
                    column("Status", VStringArray.of(status, Alarm.none(), Time.now())));
        }
    }
    ...
    public static VTable tableValueFilter(VTable table, String columnName, Object value) {
        ...
        BufferInteger indexes = new BufferInteger();
