...
public abstract class ServiceMethod {
    ...
    private final AsyncImpl asyncImpl;
    private final SyncImpl syncImpl;

    /**
     * Creates a new service method with the given description. All properties
     * are copied out of the description, guaranteeing the immutability of
     * objects of this class. Nonetheless, service method descriptions should
    ...
    public ServiceMethod(ServiceMethodDescription serviceMethodDescription) {
        ...
        this.asyncImpl = serviceMethodDescription.asyncImpl;
        this.syncImpl = serviceMethodDescription.syncImpl;

        if (asyncImpl == null && syncImpl == null) {
            throw new RuntimeException("Must implement the method \"syncExecImpl\" or \"asyncExecImpl\".");
        }
    }

    /**
     * A brief name for the service method. Used for service registration and
     * lookup.
     *
     ...
     * which will be communicated through the callbacks.
     * <p>
     * This method validates the parameters and then calls the implementation
     * method.
     *
     ...
     * <p>
     * The call should be implemented asynchronously, only one callback should
     * be called once with the result. If the method throws an exception, this
     * will be sent to the errorCallbak, you can leave exceptions go through in
     * which case no callback should be used.
     *
    ...
    private Map<String, Object> wrapAsSync(Map<String, Object> parameters) {
        ...
        try {
            asyncImpl.execute(parameters, callback, errorCallback);
        } catch (Exception ex) {
            errorCallback.accept(ex);
        }
    ...
    private void wrapAsAsync(ExecutorService executor, Map<String, Object> parameters, Consumer<Map<String, Object>> callback, Consumer<Exception> errorCallback) {
        executor.submit(new Runnable() {
            ...
            public void run() {
                try {
                    callback.accept(syncImpl.execute(parameters));
                } catch (Exception ex) {
                    errorCallback.accept(ex);
                }
            }
    ...
    public Map<String, Object> executeSync(Map<String, Object> parameters) {
        ...
        if (syncImpl != null) {
            return syncImpl.execute(parameters);
        } else if (asyncImpl != null) {
            return wrapAsSync(parameters);
        } else {
            //From constraints in constructor, not expected to reach this
    ...
    //
    //
    //TODO: are we allowed a null executor
    //TODO: Service requires a close() method to close the executor
    //How to ensure
}
